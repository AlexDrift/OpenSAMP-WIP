...
uint32_t CFilterscriptsManager::OnPlayerClickTextDraw(cell playerID, cell textDrawID)
{
	int idx;
	cell ret = 0;
	...
	for (uint32_t i = 0; i < MAX_FS; i++)
	{
		if (fsAMX[i])
		{
			if (!amx_FindPublic(fsAMX[i], "OnPlayerClickTextDraw", &idx))
			{
				amx_Push(fsAMX[i], textDrawID);
				amx_Push(fsAMX[i], playerID);
				amx_Exec(fsAMX[i], &ret, idx);
				if (ret) return 1;
			}
...
uint32_t CFilterscriptsManager::OnPlayerClickPlayerTextDraw(cell playerID, cell textDrawID)
{
	int idx;
	cell ret = 0;
	...
	for (uint32_t i = 0; i < MAX_FS; i++)
	{
		if (fsAMX[i])
		{
			if (!amx_FindPublic(fsAMX[i], "OnPlayerClickPlayerTextDraw", &idx))
			{
				amx_Push(fsAMX[i], textDrawID);
				amx_Push(fsAMX[i], playerID);
				amx_Exec(fsAMX[i], &ret, idx);
