	...
	if( __NetGame->blimitGlobalChatRadius )
	{
		for( _PlayerID playerID = 0; playerID < MAX_PLAYERS; playerID ++ )
		{
			...
			if( __NetGame->playerPool->GetSlotState( playerID ) )
			{
				...
				{
					__NetGame->PlayerRPC(RPC_Chat, &bStream, playerID, PACKET_STREAM_RPC_TEXT);
				}
			}
		}
		__NetGame->PlayerRPC(RPC_Chat, &bStream, this->myPlayerID, PACKET_STREAM_RPC_TEXT);
	}
	else
	{
		__NetGame->GlobalRPC(RPC_Chat, &bStream, INVALID_PLAYER_ID, PACKET_STREAM_RPC_TEXT);
	}
...
void CPlayer::UpdatePlayerMarkers()
{
	...
	if( (__NetGame->GetTime() - this->lastMarkerUpdate) > MARKER_UPDATE_TICKS ) 
	{
		...
		bStream.Write((uint8_t) ID_MARKERS_SYNC);
...
void CPlayer::HandleDeath(uint8_t deathReason, _PlayerID killerID )
{
	...
	if(__NetGame->gamemodeManager)
		__NetGame->gamemodeManager->OnPlayerDeath((cell)this->myPlayerID, (cell)killerID, (cell)deathReason);
