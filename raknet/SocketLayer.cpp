...
unsigned char CraftSum(char *buf, int len)
{
	char sum = 0;
	for (int i = 0; i < len; i++)
#if RAKNET_SOCKET_LAYER_PORT_REV == 1
		sum += buf[i] & RAKNET_SOCKET_LAYER_SUM_KEY;
#else
		sum ^= buf[i] & RAKNET_SOCKET_LAYER_SUM_KEY;
#endif
	return sum;
...
unsigned char usPacketTrans[512] =
{
	RAKNET_SOCKET_KEY_MATRIX
};
...
void DecodeData(unsigned char *data, int len, unsigned char key1, unsigned char key2)
{
	...
	for(int i = 0; i < len; i++)
	{
		if (key_swap_flag)
		{
#if RAKNET_SOCKET_LAYER_PORT_REV == 2
			data[i] = key1 ^ data[i];
#else
			data[i] ^= key2;
#endif
...
#if RAKNET_SOCKET_LAYER_PORT_REV == 3
		data[i] = usPacketTrans[data[i]];
#else
		data[i] = usPacketTrans[data[i] * 2 + 1];
#endif
	}
}
...
#elif RAKNET_SOCKET_LAYER_PORT_REV == 2
	DecodeData((unsigned char*)buffer, *len, (port + RAKNET_SOCKET_LAYER_PORT_ADD) ^ RAKNET_SOCKET_LAYER_PORT_KEY, 0);
#elif RAKNET_SOCKET_LAYER_PORT_REV == 3
	DecodeData((unsigned char*) buffer, *len, 0, (port + RAKNET_SOCKET_LAYER_PORT_ADD) ^ RAKNET_SOCKET_LAYER_PORT_KEY);
#endif
