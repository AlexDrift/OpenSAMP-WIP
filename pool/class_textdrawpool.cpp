...
uint16_t CTextDrawPool::Create( float x, float y, char* text ) // Ref: 0x47D0C0
{
	...
	this->textDraw[id]->SizeX = 1280.0f;
	this->textDraw[id]->SizeY = 1280.0f;
	this->textDraw[id]->modelid = 0;
	this->textDraw[id]->rotX = 0;
	this->textDraw[id]->rotY = 0;
	this->textDraw[id]->rotZ = 0;
	this->textDraw[id]->color1 = 0xFFFF;
	this->textDraw[id]->color2 = 0xFFFF;
	this->textDraw[id]->textdrawFlags = 0xF0 | 0x10;
	...
	this->textDraw[id]->useShadow = 2;
	this->textDraw[id]->useOutline = 0;
	this->textDraw[id]->textFont = 1;
	this->textDraw[id]->selectable = 0;
	this->textDraw[id]->posX = x;
	this->textDraw[id]->posY = y;
	this->textDraw[id]->zoom = 1.0f;
	this->isCreated[id] = 1;
...
int CTextDrawPool::setSelectable(uint16_t textDrawID, bool selectable)
{
	if (0 > textDrawID || textDrawID >= MAX_TEXTDRAW) return 0;
	if (this->isCreated[textDrawID] == 0) return 0;

	textDraw[textDrawID]->selectable = selectable;
	return 1;
}

int CTextDrawPool::setModel(uint16_t textDrawID, WORD modelid)
{
	if (0 > textDrawID || textDrawID >= MAX_TEXTDRAW) return 0;
	if (this->isCreated[textDrawID] == 0) return 0;

	textDraw[textDrawID]->modelid = modelid;
	return 1;
}

int CTextDrawPool::setRot(uint16_t textDrawID, float rotX, float rotY, float rotZ, float zoom)
{
	if (0 > textDrawID || textDrawID >= MAX_TEXTDRAW) return 0;
	if (this->isCreated[textDrawID] == 0) return 0;

	textDraw[textDrawID]->rotX = rotX;
	textDraw[textDrawID]->rotY = rotY;
	textDraw[textDrawID]->rotZ = rotZ;
	textDraw[textDrawID]->zoom = zoom;
	return 1;
}

int CTextDrawPool::setVehColor(uint16_t textDrawID, WORD color1, WORD color2)
{
	if (0 > textDrawID || textDrawID >= MAX_TEXTDRAW) return 0;
	if (this->isCreated[textDrawID] == 0) return 0;

	textDraw[textDrawID]->color1 = color1;
	textDraw[textDrawID]->color2 = color2;
	return 1;
}
